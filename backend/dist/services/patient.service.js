...
class PatientService {
    static async listPatients(hospitalId, search) {
        return client_1.prisma.patient.findMany({
            where: {
                hospitalId,
                ...(search
                    ? {
    ...
    static async getPatientById(hospitalId, id) {
        const patient = await client_1.prisma.patient.findFirst({
            where: { id, hospitalId },
            include: {
    ...
    static async registerPatient(hospitalId, data) {
        // Generate unique MRN
        const count = await client_1.prisma.patient.count({ where: { hospitalId } });
        const mrn = `MRN-${new Date().getFullYear()}-${(count + 1).toString().padStart(4, '0')}`;
        return client_1.prisma.patient.create({
            data: {
                hospitalId,
                mrn,
    ...
    static async recordVitals(patientId, recordedByUserId, data) {
        return client_1.prisma.vital.create({
            data: {
                patientId,
