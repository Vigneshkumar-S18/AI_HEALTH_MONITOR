...
export class ACIService {
  static async startSession(hospitalId: string, appointmentId: string, doctorId: string, patientId: string) {
    const existing = await prisma.aCISession.findUnique({
      where: { appointmentId },
    ...
    return prisma.aCISession.create({
      data: {
        hospitalId,
        appointmentId,
        doctorId,
  ...
  static async generateNoteAndCodes(sessionId: string) {
    const session = await prisma.aCISession.findUnique({ where: { id: sessionId } });
    if (!session) throw { statusCode: 404, message: 'ACI session not found.' };
    ...
    await prisma.aCISession.update({
      where: { id: sessionId },
      data: {
    ...
    const noteDraft = await prisma.clinicalNoteDraft.upsert({
    ...
    await prisma.codingSuggestion.deleteMany({ where: { aciSessionId: sessionId } });

    const codes = await prisma.codingSuggestion.createMany({
      data: [
        {
          aciSessionId: sessionId,
          codeSystem: 'ICD-10',
